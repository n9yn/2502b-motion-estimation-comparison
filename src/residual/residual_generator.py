...
def generate_residual_frame(
    ...
    reference = _to_grayscale(reference_frame)
    residual = np.zeros(target.shape, dtype=np.int16)
    ...
    for vector in motion_vectors:
        ...
        if (
            ...
            or src_x + block_size > reference.shape[1]
            or src_y + block_size > reference.shape[0]
            or dst_x + block_size > target.shape[1]
            or dst_y + block_size > target.shape[0]
        ):
            continue

        reference_block = reference[src_y:src_y + block_size, src_x:src_x + block_size].astype(np.int16)
        target_block = target[dst_y:dst_y + block_size, dst_x:dst_x + block_size].astype(np.int16)
        residual[dst_y:dst_y + block_size, dst_x:dst_x + block_size] = target_block - reference_block
