...
def _search_candidates(
    ...
    """Search a set of candidate displacements and return best dx, dy, metric distance, comparisons."""
    best_dx = center_dx
    best_dy = center_dy
...
def diamond_search_motion_estimation(
    ...
    for y in range(0, height - block_size + 1, block_size):
        for x in range(0, width - block_size + 1, block_size):
            ...
            while True:
                best_dx, best_dy, best_distance, comps = _search_candidates(
                    reference_block,
                    target_int,
                    x,
                    y,
                    ...
                    metric,
                    use_numba,
                    offsets=_get_large_diamond_offsets(),
                )
                comparisons += comps

                if best_dx != center_dx or best_dy != center_dy:
                ...
                best_dx, best_dy, best_distance, comps = _search_candidates(
                    ...
                    metric,
                    use_numba,
                    offsets=_get_small_diamond_offsets(),
                )
                comparisons += comps
